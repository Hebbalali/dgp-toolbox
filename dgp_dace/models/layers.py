...
#  def sample_Z_right(layers,Z,S=10):
#      Z = tf.tile(tf.expand_dims(Z, 0), [S, 1, 1])
#      for i,layer in enumerate(layers):
#          if i==0:
#              Z_right = sample(layer,Z)
#          Z_aug = tf.concat([Z, Z_right], 2)
#          Z_right = sample(layer,Z_aug)
#      return Z_right

# @params_as_tensors
# def sample_Z_right(layers,Z,S):
#     Z = tf.tile(tf.expand_dims(Z, 0), [S, 1, 1])
#     for i,layer in enumerate(layers):
#         if i==0:
#             Z_right = tf.reduce_mean(layer.sample_from_conditional(Z)[0],axis=0)
#         Z_aug = tf.concat([Z, Z_right], 2)
#         Z_right = tf.reduce_mean(layer.sample_from_conditional(Z_aug)[0],axis=0)
#     return Z_right


# def sample_Z_right(layers,Z,S):
#     Z = tf.tile(tf.expand_dims(Z, 0), [S, 1, 1])
#     Z_right = sample(layers[0],Z)
#     # Z_right = tf.reduce_mean(layers.sample_from_conditional(Z)[0],axis=0)
#     return Z_right
...
def sample_Z_right_array_all_layers(layers,Z,S):
    for i,layer in enumerate(layers):
        if i==0:
...
class SVGP_Layer(Layer):
    def __init__(self, kern, Z, num_outputs, mean_function,augmented=False,layers=None,
        ...
        if augmented==False:
            ...
            self.feature = InducingPoints(layers=layers,Z=Z)
            Z_right = sample_Z_right_array_all_layers(layers,Z.numpy(),100)
